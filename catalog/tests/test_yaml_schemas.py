...
class YAMLSchemaTest(unittest.TestCase):
    def test_gpu_yaml_rejects_invalid_vendor(self) -> None:
    ...
    def test_gpu_yaml_accepts_valid_payload(self) -> None:
        gpu = GPUYAML(
        ...
        self.assertEqual(gpu.slug, "nvidia-h100-sxm-80")

    def test_model_yaml_rejects_dense_with_mismatched_params(self) -> None:
        with self.assertRaises(ValidationError):
            ModelYAML(
                slug="m",
                display_name="M",
    ...
    def test_quantization_yaml_rejects_invalid_bits(self) -> None:
        with self.assertRaises(ValidationError):
