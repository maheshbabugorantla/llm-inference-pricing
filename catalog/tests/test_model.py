...
class ModelTest(TestCase):
    def test_recommended_quant_fk_traversal_provides_weight_bits_for_fit(self):
        ...
        self.assertEqual(model.recommended_quant.slug, "fp8-e4m3")
        self.assertEqual(model.recommended_quant.weight_bits, 8)

    def test_model_slug_must_be_unique(self):
    ...
    def test_dense_model_rejects_mismatched_active_and_total_params(self):
    ...
    def test_recommended_tp_must_be_power_of_two_from_valid_set(self):
    ...
    def test_moe_model_stores_active_and_total_params_independently(self):
        ...
        self.assertEqual(fetched.total_params_b, 671.0)
        self.assertEqual(fetched.active_params_b, 37.0)
        self.assertLess(fetched.active_params_b, fetched.total_params_b)


class ModelFitTest(unittest.TestCase):
    """Pure-Python fit computation tests — no DB access needed."""

    def test_moe_model_requires_all_experts_loaded_not_just_active_params(self):
        ...
        self.assertTrue(fits_active, "37B active params should fit on 80GB GPU")
        self.assertFalse(fits_total, "671B total MoE params must not fit on a single 80GB GPU")

    def test_model_context_length_drives_kv_cache_fit_boundary(self):
        ...
        self.assertTrue(fits_4k, "Qwen-32B fp16 tp=1 should fit on H100 80GB at ctx=4k")
        self.assertFalse(fits_128k, "Qwen-32B fp16 tp=1 should NOT fit on H100 80GB at ctx=128k batch=8")

    def test_tp4_sharding_enables_larger_batch_to_fit_same_gpu_vram(self):
        ...
        self.assertFalse(fits_tp1, "Qwen-32B fp16 batch=64 must not fit on single H100 80 GB (tp=1)")
        self.assertTrue(fits_tp4, "Qwen-32B fp16 batch=64 should fit on H100 80 GB with tp=4 sharding")

    def test_model_with_more_layers_has_proportionally_larger_kv_cache_footprint(self):
