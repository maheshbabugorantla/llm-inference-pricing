...
@pytest.mark.django_db
def test_qwen32b_fp16_fits_a100_80gb_but_not_a100_40gb():
    ...
    assert fits_80, "Qwen-32B fp16 should fit the 80 GB A100"
    assert not fits_40, "Qwen-32B fp16 must not fit the 40 GB A100"
    assert gpu_80.vram_gb == 80
    assert gpu_40.vram_gb == 40


@pytest.mark.django_db
def test_inactive_gpu_historical_snapshots_remain_queryable():
    ...
    assert PricingSnapshot.objects.filter(pk=snap.pk).exists()
    assert PricingSnapshot.objects.get(pk=snap.pk).gpu.is_active is False


@pytest.mark.django_db
def test_gpu_meta_ordering_is_vendor_then_slug_for_deterministic_pipeline_queries():
    ...
    assert amd_idx < nvidia_h100_idx, "AMD must sort before NVIDIA (vendor='amd' < 'nvidia')"
    assert nvidia_a100_idx < nvidia_h100_idx, "a100 must sort before h100 within nvidia"
