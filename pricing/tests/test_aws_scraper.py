"""Tests for AWS GPU pricing scraper (M05.5b.T02)."""

from __future__ import annotations

import json
import logging
import unittest
from decimal import Decimal
from pathlib import Path
from unittest.mock import MagicMock, patch

from pricing.scrapers.aws import (
    _CAPACITY_BLOCK_DURATIONS_HOURS,
    _CAPACITY_BLOCK_SKIP_CODES,
    fetch_aws_capacity_blocks,
    parse_aws_capacity_blocks,
    parse_aws_prices,
)
from pricing.scrapers.base import ParserDriftError

FIXTURES = Path(__file__).parent / "fixtures"


def _mock_ec2_client(paginator: MagicMock) -> MagicMock:
    ec2 = MagicMock()
    ec2.get_paginator.return_value = paginator
    return ec2


class AwsParserTest(unittest.TestCase):
    def setUp(self) -> None:
        self.p5_products = json.loads((FIXTURES / "aws_pricing_p5.json").read_text())
        self.capacity_blocks = json.loads((FIXTURES / "aws_capacity_blocks_p5.json").read_text())

    def test_aws_parse_p5_yields_per_gpu_hourly_after_dividing_by_8(self) -> None:
        prices = parse_aws_prices(self.p5_products)
        p5 = next(p for p in prices if p.gpu_slug_hint == "nvidia-h100-sxm-80")
        # p5.48xlarge $98.3232 / 8 GPUs = $12.2904 per GPU
        self.assertEqual(p5.hourly_usd, Decimal("98.3232") / Decimal(8))
        self.assertEqual(p5.tier, "on_demand")
        self.assertEqual(p5.provider_slug, "aws")

    def test_aws_parse_p4d_yields_per_gpu_hourly_after_dividing_by_8(self) -> None:
        prices = parse_aws_prices(self.p5_products)
        p4d = next(p for p in prices if "a100-sxm-40" in p.gpu_slug_hint)
        # p4d.24xlarge $32.7726 / 8 GPUs
        self.assertEqual(p4d.hourly_usd, Decimal("32.7726") / Decimal(8))

    def test_aws_parse_g5_yields_per_gpu_hourly_after_dividing_by_8(self) -> None:
        prices = parse_aws_prices(self.p5_products)
        g5 = next(p for p in prices if "a10g" in p.gpu_slug_hint)
        # g5.48xlarge $16.2880 / 8 GPUs
        self.assertEqual(g5.hourly_usd, Decimal("16.2880") / Decimal(8))

    def test_aws_capacity_block_amortizes_upfront_over_duration(self) -> None:
        prices = parse_aws_capacity_blocks(self.capacity_blocks)
        self.assertEqual(len(prices), 2)
        # First offering: $131712 upfront / (2 instances * 336 hours) = $196/instance-hr / 8 GPUs
        cb = next(p for p in prices if "336" in str(p.raw.get("duration_hours", "")))
        expected_per_instance = Decimal("131712.00") / (Decimal(2) * Decimal(336))
        expected_per_gpu = expected_per_instance / Decimal(8)
        self.assertEqual(cb.hourly_usd, expected_per_gpu)
        self.assertTrue(cb.tier.startswith("capacity-block-"))
        self.assertIn("14d", cb.tier)  # 336 hours = 14 days

    def test_aws_capacity_block_7d_tier_name(self) -> None:
        prices = parse_aws_capacity_blocks(self.capacity_blocks)
        # Second offering: 168 hours = 7 days
        cb = next(p for p in prices if "168" in str(p.raw.get("duration_hours", "")))
        self.assertEqual(cb.tier, "capacity-block-7d")

    def test_aws_parse_handles_missing_instance_type_gracefully(self) -> None:
        unknown_product = [
            {
                "attributes": {"instanceType": "m5.xlarge", "regionCode": "us-east-1"},
                "terms": {"OnDemand": {"k": {"priceDimensions": {"kk": {"pricePerUnit": {"USD": "0.192"}}}}}},
            }
        ]
        with self.assertRaises(ParserDriftError):
            parse_aws_prices(unknown_product)

    def test_aws_parser_returns_decimals(self) -> None:
        prices = parse_aws_prices(self.p5_products)
        for p in prices:
            self.assertIsInstance(p.hourly_usd, Decimal)

    def test_unmapped_gpu_instance_type_is_skipped_and_logged(self) -> None:
        """An unknown GPU instance type (p99.huge) must be skipped — no snapshot row
        created — and logged so operators can add it to the mapping table. Silently
        dropping it without a log would hide a pricing gap from the oncall dashboard."""
        unknown = [
            {
                "attributes": {"instanceType": "p99.huge", "regionCode": "us-east-1"},
                "terms": {"OnDemand": {"k": {"priceDimensions": {"kk": {"pricePerUnit": {"USD": "999"}}}}}},
            },
            *self.p5_products,
        ]
        with self.assertLogs("pricing.scrapers.aws", level=logging.INFO) as cm:
            prices = parse_aws_prices(unknown)

        output = "\n".join(cm.output)
        self.assertIn("p99.huge", output, "unknown instance must be logged")
        self.assertTrue(
            all(p.raw.get("instance_type") != "p99.huge" for p in prices),
            "unknown instance must not appear in results",
        )
        self.assertEqual(
            len(prices), len(parse_aws_prices(self.p5_products)), "known instances must all be present"
        )

    def test_aws_empty_capacity_blocks_returns_empty_list(self) -> None:
        self.assertEqual(parse_aws_capacity_blocks([]), [])

    def test_p5e_and_p5_both_map_to_h100_slug_but_are_priced_independently(self) -> None:
        """p5e.48xlarge is a newer H100 instance type alongside the original p5.48xlarge.
        Both must parse to the same gpu_slug_hint but as separate ScrapedPrice objects —
        if one silently overwrote the other, one price variant would be lost."""
        products = [
            {
                "attributes": {"instanceType": "p5.48xlarge", "regionCode": "us-east-1"},
                "terms": {
                    "OnDemand": {"k1": {"priceDimensions": {"d1": {"pricePerUnit": {"USD": "98.3232"}}}}}
                },
            },
            {
                "attributes": {"instanceType": "p5e.48xlarge", "regionCode": "us-east-1"},
                "terms": {
                    "OnDemand": {"k2": {"priceDimensions": {"d2": {"pricePerUnit": {"USD": "110.0000"}}}}}
                },
            },
        ]
        prices = parse_aws_prices(products)
        h100_prices = [p for p in prices if p.gpu_slug_hint == "nvidia-h100-sxm-80"]
        self.assertEqual(len(h100_prices), 2, "Both p5 and p5e must produce independent H100 price rows")
        hourly_values = {p.hourly_usd for p in h100_prices}
        self.assertEqual(len(hourly_values), 2, "p5 and p5e must have distinct per-GPU hourly rates")

    def test_g5_xlarge_single_gpu_price_is_not_divided(self) -> None:
        """g5.xlarge has exactly 1 GPU, so the instance price IS the per-GPU price —
        it must not be divided by 8. Applying an 8-GPU divisor to a 1-GPU instance
        would understate AWS A10G pricing by 8x."""
        instance_hourly = Decimal("1.006")
        product = [
            {
                "attributes": {"instanceType": "g5.xlarge", "regionCode": "us-east-1"},
                "terms": {
                    "OnDemand": {
                        "k": {"priceDimensions": {"d": {"pricePerUnit": {"USD": str(instance_hourly)}}}}
                    }
                },
            }
        ]
        prices = parse_aws_prices(product)
        self.assertEqual(len(prices), 1)
        self.assertEqual(
            prices[0].hourly_usd,
            instance_hourly,
            f"g5.xlarge (1 GPU) per-GPU price must equal instance price {instance_hourly}",
        )

    def test_all_aws_on_demand_parsed_prices_have_correct_provider_and_tier(self) -> None:
        """Every ScrapedPrice from parse_aws_prices must carry provider_slug='aws' and
        tier='on_demand'. A wrong provider slug would route prices to the wrong DB rows;
        a wrong tier would misclassify on-demand as spot or reserved in cost cells."""
        prices = parse_aws_prices(self.p5_products)
        self.assertGreater(len(prices), 0)
        for price in prices:
            self.assertEqual(
                price.provider_slug, "aws", f"Expected provider 'aws', got {price.provider_slug!r}"
            )
            self.assertEqual(price.tier, "on_demand", f"Expected tier 'on_demand', got {price.tier!r}")


class AwsCapacityBlockFetchTest(unittest.TestCase):
    def test_fetch_aws_capacity_blocks_passes_duration_to_every_paginator_call(self) -> None:
        """Each describe_capacity_block_offerings call must include CapacityDurationHours.

        Omitting it causes ParamValidationError at runtime. This test records every
        paginator.paginate() call and verifies the required param is always present.
        """
        captured_calls: list[dict] = []

        def fake_paginate(**kwargs: object) -> list:
            captured_calls.append(dict(kwargs))
            return []

        mock_paginator = MagicMock()
        mock_paginator.paginate.side_effect = fake_paginate

        with patch("boto3.client", return_value=_mock_ec2_client(mock_paginator)):
            fetch_aws_capacity_blocks()

        self.assertEqual(
            len(captured_calls),
            len(_CAPACITY_BLOCK_DURATIONS_HOURS),
            "Expected one paginator call per duration",
        )
        for call_kwargs in captured_calls:
            self.assertIn(
                "CapacityDurationHours",
                call_kwargs,
                f"CapacityDurationHours missing from paginator call: {call_kwargs}",
            )
        self.assertEqual(
            {c["CapacityDurationHours"] for c in captured_calls},
            set(_CAPACITY_BLOCK_DURATIONS_HOURS),
        )

    def test_fetch_aws_capacity_blocks_skips_expected_client_errors_per_duration(self) -> None:
        """UnsupportedOperation and similar 'no offerings' codes must be skipped per-duration.

        Auth/quota errors (not in _CAPACITY_BLOCK_SKIP_CODES) must be re-raised so
        callers see the real failure rather than silently getting an empty list.
        """
        import botocore.exceptions

        call_count = 0

        def fake_paginate(**kwargs: object) -> list:
            nonlocal call_count
            call_count += 1
            raise botocore.exceptions.ClientError(
                {"Error": {"Code": "UnsupportedOperation", "Message": "not available"}},
                "DescribeCapacityBlockOfferings",
            )

        mock_paginator = MagicMock()
        mock_paginator.paginate.side_effect = fake_paginate

        with patch("boto3.client", return_value=_mock_ec2_client(mock_paginator)):
            result = fetch_aws_capacity_blocks()

        self.assertEqual(result, [], "Expected empty list when all durations return UnsupportedOperation")
        self.assertEqual(call_count, len(_CAPACITY_BLOCK_DURATIONS_HOURS))

    def test_fetch_aws_capacity_blocks_reraises_unexpected_client_errors(self) -> None:
        """Non-skip ClientErrors (e.g. AuthFailure) must propagate, not be swallowed."""
        import botocore.exceptions

        def fake_paginate(**kwargs: object) -> list:
            raise botocore.exceptions.ClientError(
                {"Error": {"Code": "AuthFailure", "Message": "credentials invalid"}},
                "DescribeCapacityBlockOfferings",
            )

        mock_paginator = MagicMock()
        mock_paginator.paginate.side_effect = fake_paginate

        with patch("boto3.client", return_value=_mock_ec2_client(mock_paginator)):
            with self.assertRaises(botocore.exceptions.ClientError) as ctx:
                fetch_aws_capacity_blocks()

        self.assertEqual(ctx.exception.response["Error"]["Code"], "AuthFailure")
        self.assertNotIn("AuthFailure", _CAPACITY_BLOCK_SKIP_CODES)
