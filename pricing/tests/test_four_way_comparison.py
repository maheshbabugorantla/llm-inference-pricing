...
PRD §7.6 Appendix A: Qwen2.5-Coder-32B FP8 at batch=8 ctx=32k must surface all four
deployment modes simultaneously in pricing_current_cost_cells:
  - on-demand cloud (community tier from a cloud scrape)
  - reserved cloud  (reserved-<slug> tier from the generator)
  - on-prem TCO     (tco tier from the on-prem generator)
  - on-prem marginal (marginal tier from the on-prem generator)

The ordering invariant from the PRD: marginal < tco (sunk capex is cheaper than green-field).
"""
...
def test_appendix_a_four_way_comparison():
    ...
    PRD §7.6 Appendix A — every cost comparison page must show cloud on-demand, reserved cloud,
    ...
    assert "community" in tiers, f"on-demand cloud tier 'community' missing from cost cells. Found: {tiers}"
    assert any(t.startswith("reserved-") and not t.startswith("reserved-marginal-") for t in tiers), (
        f"reserved cloud tier (reserved-<slug>) missing from cost cells. Found: {tiers}"
    )
    assert "tco" in tiers, f"on-prem TCO tier missing from cost cells. Found: {tiers}"
    assert "marginal" in tiers, f"on-prem marginal tier missing from cost cells. Found: {tiers}"
    ...
    assert marginal_costs and tco_costs, "Need at least one marginal and one tco row"
    assert min(marginal_costs) < min(tco_costs), (
