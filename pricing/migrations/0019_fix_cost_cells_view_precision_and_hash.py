...
# Fixes three correctness issues in the materialized view introduced in 0016:
#
# 1. row_hash collision risk: bare concatenation (a||b||c) is ambiguous
#    ("ab"+"c" == "a"+"bc"). Add '|' delimiters so every component boundary
#    is unambiguous and the unique index is guaranteed correct.
#
...
#
# reverse_sql restores the pre-0019 state: the 0016 view definition (float
# literals, no delimiters, raw tps columns) plus the 0018 composite indexes.
...
# Pre-0019 view definition (from migration 0016): float literals, no delimiters,
# raw float TPS columns. Used in reverse_sql to restore the state that 0018 left.
OLD_VIEW_SQL = """
CREATE MATERIALIZED VIEW pricing_current_cost_cells AS
WITH latest AS (
    SELECT DISTINCT ON (provider_id, gpu_id, tier, region)
...
# Index set that existed after 0018 (base from 0016, composite from 0018;
# pricing_cost_cells_output_cost was dropped by 0018 so it is NOT recreated here).
OLD_INDEXES = """
CREATE UNIQUE INDEX pricing_cost_cells_uniq
    ON pricing_current_cost_cells (row_hash);
CREATE INDEX pricing_cost_cells_model
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunSQL(
            ...
            reverse_sql=DROP_INDEXES + OLD_VIEW_SQL + OLD_INDEXES,
        ),
    ]
